...
oc new-app --name webserver-app3 --image registry.ocp4.example.com:8443/redhattraining/hello-world-nginx:v1.0
oc expose service webserver-app3
oc patch deployment webserver-app3 -n chapter3 --type=json -p='[{"op": "add", "path": "/spec/template/spec/nodeSelector","value":{"disktype":"ssd"}}]'

```


# Question 3. An application is running on the `chapter2` project. There is one pod, which is in pending state. Your task is it must generate the output.
---
### Solution
### Check the resources.
```
